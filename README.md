...
**Status:** early development. Player movement, static AABB collision, and deterministic procedural platform generation are implemented.
...
Move with **A/D** or **Left/Right**, jump with **Space**, and exit with **Escape**. Opposite directions cancel and releasing movement stops on the next simulation step. A fresh Space press jumps only while grounded; holding it does not auto-jump on landing. The 40 x 64 player spawns above the generated starting platform. Falling below center Y = -480 resets position, velocity, and grounded state to that level's spawn; there is no camera tracking.
...
`generateLevel(seed)` uses a local `std::mt19937` and a fixed integer mapping. The same seed produces the same layout without using time. Ten platforms form a guaranteed reachable primary route from left to right: a wide start, varied 48–64-unit platforms, and an amber goal marker with no win state. Candidates must pass reachability and layout checks, with at most 32 attempts per placement and a validated same-height fallback. Additional height-step limits keep nearby arcs clear. The level fits the default 1280 x 720 view; narrower aspect ratios can crop its edges. The runtime development seed is `20260909` in `LevelGenerator.hpp`; relaunching repeats it. `TestLevel.hpp` remains a collision-test fixture only.
...
To check the foundation, let the mint-colored player land on the wide starting platform, then jump right along the route toward the amber final platform. Walk off the left edge to test falling and reset. Resize the window to check proportions, then press Escape to close it. Build output, downloaded dependencies, and generated loader files are ignored by Git.

CTest runs `PlayerPhysics`, `StaticPlatformCollision`, `JumpReachability`, and `SeededLevelGeneration` without a window. Existing movement/collision regressions remain, with analytic reachability tests and an invariant sweep over seeds 0–999. The sweep also attempts all 9,000 generated jumps with the real 120 Hz Player and collision simulation, including upward, downward, and same-height transitions. The analytic utility assumes an unobstructed arc; this second layer checks actual route geometry and discrete integration. Configure with `-DBUILD_TESTING=OFF` to omit tests.
