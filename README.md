...
**Status:** early development. Player movement, static AABB collision, deterministic procedural generation, and a scrolling 2D camera are implemented.
...
Move with **A/D** or **Left/Right**, jump with **Space**, and exit with **Escape**. Opposite directions cancel and releasing movement stops on the next simulation step. A fresh Space press jumps only while grounded; holding it does not auto-jump on landing. The 40 x 64 player spawns above the generated starting platform. Falling below center Y = -480 resets position, velocity, and grounded state to that level's spawn and recenters the camera. The fixed reset threshold remains below the unchanged vertical generation range.
...
To check the foundation, let the mint-colored player land on the wide starting platform, then jump right along the scrolling route toward the amber final platform. Stop to check camera stability, walk off an edge to test player/camera reset, and resize while traversing. Press Escape to close it. Build output, downloaded dependencies, and generated loader files are ignored by Git.

CTest runs `PlayerPhysics`, `StaticPlatformCollision`, `JumpReachability`, `SeededLevelGeneration`, and `Camera2D` without a window. Existing movement/collision regressions remain, with analytic reachability tests and an invariant sweep over seeds 0–999. The sweep checks world bounds and attempts all 27,000 generated jumps with the real 120 Hz Player and collision simulation, including upward, downward, and same-height transitions. The analytic utility assumes an unobstructed arc; this second layer checks actual route geometry and discrete integration. Camera tests cover following, clamps, matrices, resize, reset, finite/deterministic calculations, and physics independence. Configure with `-DBUILD_TESTING=OFF` to omit tests.
