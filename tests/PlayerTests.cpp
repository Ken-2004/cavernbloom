...
constexpr float restingY = -240.0F + simulation::playerSize.y * 0.5F;
...
void checkJumping()
{
    ...
    while (player.position().y > restingY && airborneTicks < 120) {
        ...
        player.update(step, 0, true, platforms);
        baseline.update(step, 0, false, platforms);
    ...
    for (const int framesPerSecond : std::array{30, 60, 144, 240}) {
        ...
        for (int frame = 0; frame < framesPerSecond * 2; ++frame) {
            ...
            while (accumulator >= simulation::fixedStepSeconds) {
                player.update(step, 1, jumpPending, platforms);
                jumpPending = false;
